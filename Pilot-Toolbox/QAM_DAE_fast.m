function [SignalOut, varargout] = QAM_DAE_fast(SignalIn, Training, P)
% Fully data aided least mean squares.
% Updated on symbol based. Can be used with all M-QAM. 
% 
% [SignalOut, varargout] = QAM_DAE_fast(SignalIn, P)
%
% Inputs:
...
if isfield(P,'GaussianWindow')
    window = gausswin(P.FilterLength);
elseif isfield(P,'ChebWin')
    window = chebwin(P.FilterLength);
else
    window = ones(1,P.FilterLength);
end
...
% normalised LMS has the update parameter scaled with the norm of the input
% Ref: Ch6 from Haykin, "Adaptive Filter Theory" (2002) 
if isfield(P,'Normalised')&&P.Normalised
    norm = cumsum(abs(SignalIn.Et).^2,2);
    norm(:,P.FilterLength+1:end) = norm(:,P.FilterLength+1:end) -norm(:,1:end-P.FilterLength) ; 
    norm = sum(norm,1);
    norm = 1./norm;
else
    norm = ones(1,size(SignalIn.Et,2));
end

% set norm to zero to not update taps
if isfield(P,'ExIdxRDEupdate')
    norm = norm.*P.ExIdxRDEupdate;
end

if size(Training.Et,2)<size(SignalIn.Et,2)
    error('Provide training data for whole sequence');
end

%% Equalizer (call mex function)
% N.B the interleave blocks simplify the interface with C++
[fout2n1,fout2n2,ts1,ts2,H11,H12,H21,H22]=interleave(1,SignalIn.Et(1,:),SignalIn.Et(2,:),Training.Et(1,:),Training.Et(2,:),H11,H12,H21,H22);
for index = 1:length(P.mu)
    if (length(P.mu)>1&&isfield(P,'verbose')&&P.verbose>=1), fprintf('.'); end
    
    mu = P.mu(index).*norm;
    [out1,out2] = dae_qam_mex(fout2n1,fout2n2,ts1,ts2,H11,H12,H21,H22,radii,mu,window,P.Threads);
end
