 ...
 *      rde_qam_mex(ETL,ETR,H11,H12,H21,H22,radii,mu,window,threads);
 *
...
#define	TSL_IN		prhs[2]
#define	TSR_IN		prhs[3]
#define	HXX_IN		prhs[4]
#define	HXY_IN		prhs[5]
#define	HYX_IN		prhs[6]
#define	HYY_IN		prhs[7]
#define	RADII_IN	prhs[8]
#define	MU_IN		prhs[9]
#define	WINDOW_IN	prhs[10]
#define	THREADS_IN	prhs[11]
...
//hxx,hxy,etl,etr,tsl,etl_o,nf,error1,mu,window,radii,radii_length,start,n
/* adaptive filter: bulk of work is done here */
void afir(cmp* h11, cmp* h12, cmp* x1, cmp* x2, cmp* y, cmp* yhat, 
        int nf, double* mu, double* window, 
        double radii[], int radii_length, int start, int n) {
    cmp temp;
    for(int s=start; s<n; s=s+2){
        /* filter x with h using nf taps starting on point s */
        /* symbol */
        firfilt(h11,x1,yhat,s,nf);
        firfilt(h12,x2,yhat,s,nf);
        /* transition */
        firfilt(h11,x1,yhat,s-1,nf); 
        firfilt(h12,x2,yhat,s-1,nf);
                     
        /* filter tap weight updates */
        temp = mu[s]*(y[s] - yhat[s]);
        for(int j=0;j<nf;++j){
            h11[j]+= window[j]*temp*conj(x1[j+s-nf+1]);
            h12[j]+= window[j]*temp*conj(x2[j+s-nf+1]);
        }
    }
}
        ...
        int nrhs, const mxArray*prhs[] )
{
    ...
    int threads;
    cmp *etl, *etr, *tsl, *tsr, *etl_o, *etr_o, *hxx, *hxy, *hyx, *hyy;
...
//     m = mxGetM(ETL_IN);
    ...
    tsl = (cmp*) mxGetPr(TSL_IN);
    tsr = (cmp*) mxGetPr(TSR_IN);
    ...
    if (threads>1){
        ...
        t[0] = thread(afir,hxx,hxy,etl,etr,tsl,etl_o,nf,mu,window,radii,radii_length,start,n); // par x-pol
        t[1] = thread(afir,hyy,hyx,etr,etl,tsr,etr_o,nf,mu,window,radii,radii_length,start,n); // par y-pol
        
        for (int j = 0; j<2; ++j) t[j].join(); // join threads here (it'll crash if you don't)
    }else{
    	/* single thread */
        afir(hxx,hxy,etl,etr,tsl,etl_o,nf,mu,window,radii,radii_length,start,n); // x-pol
        afir(hyy,hyx,etr,etl,tsr,etr_o,nf,mu,window,radii,radii_length,start,n); // y-pol
    }
