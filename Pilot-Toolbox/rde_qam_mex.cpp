...
#define	MU_IN		prhs[7]
#define	WINDOW_IN	prhs[8]
#define	THREADS_IN	prhs[9]
...
void afir(cmp* h11, cmp* h12, cmp* x1, cmp* x2, int nf,
        cmp* y, double* error, double* mu, double* window, double radii[],
    ...
    for(int s=start; s<n; s=s+2){
        ...
        /* compute error term */
        error[s] = pow(radii[minLocFunc(abs(y[s]),radii,radii_length)],2)-norm(y[s]);
        
        /* filter tap weight updates */
        temp = mu[s]*error[s]*y[s];
        for(int j=0;j<nf;++j){
...
//     m = mxGetM(ETL_IN);
    ...
    if (threads>1){
        ...
        t[0] = thread(afir,hxx,hxy,etl,etr,nf,etl_o,error1,mu,window,radii,radii_length,start,n); // x-pol
        t[1] = thread(afir,hyy,hyx,etr,etl,nf,etr_o,error2,mu,window,radii,radii_length,start,n); // y-pol
        
        for (int j = 0; j<2; ++j) t[j].join(); // join threads here (it'll crash if you don't)
    }else{
    	/* single thread */
        afir(hxx,hxy,etl,etr,nf,etl_o,error1,mu,window,radii,radii_length,start,n); // x-pol
        afir(hyy,hyx,etr,etl,nf,etr_o,error2,mu,window,radii,radii_length,start,n); // y-pol
    }
