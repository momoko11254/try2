 ...
 *
 * [out1,out2,out3,out4,error1,error2,H11,H12,H13,H14,H21,H22,H23,H24,H31,H32,H33,H34,H41,H42,H43,H44,phase1,phase2] = 
 *      pde_qam_4x4_mex(fout2n1,fout2n2,fout2n3,fout2n4,H11,H12,H13,H14,H21,H22,H23,H24,H31,H32,H33,H34,H41,H42,H43,H44,points,cpewindow,mu);
 *
...
#define	CPE_IN		prhs[21]
#define	MU_IN		prhs[22]
...
#define	PHASE1_OUT	plhs[22]
#define	PHASE2_OUT	plhs[23]
...
{ 
    ...
    int m,n,mf,nf,points_length, cpewindow;
    ...
    for(int s=start; s<end; s=s+2){
        ...
        for(int ss=s; ss<(s+2*cpewindow); ss=ss+2){
            ...
            for(int j=0;j<nf;j++){
            /* ...on symbol */
                temp1=temp1+h11[j]*et1r[j+ss-nf+1]+h12[j]*et1i[j+ss-nf+1]
                           +h13[j]*et2r[j+ss-nf+1]+h14[j]*et2i[j+ss-nf+1];

                temp2=temp2+h21[j]*et1r[j+ss-nf+1]+h22[j]*et1i[j+ss-nf+1]
                           +h23[j]*et2r[j+ss-nf+1]+h24[j]*et2i[j+ss-nf+1];

                temp3=temp3+h31[j]*et1r[j+ss-nf+1]+h32[j]*et1i[j+ss-nf+1]
                           +h33[j]*et2r[j+ss-nf+1]+h34[j]*et2i[j+ss-nf+1];

                temp4=temp4+h41[j]*et1r[j+ss-nf+1]+h42[j]*et1i[j+ss-nf+1]
                           +h43[j]*et2r[j+ss-nf+1]+h44[j]*et2i[j+ss-nf+1];

           /* ...on transition */
                temp5=temp5+h11[j]*et1r[j+ss-nf+2]+h12[j]*et1i[j+ss-nf+2]
                           +h13[j]*et2r[j+ss-nf+2]+h14[j]*et2i[j+ss-nf+2];

                temp6=temp6+h21[j]*et1r[j+ss-nf+2]+h22[j]*et1i[j+ss-nf+2]
                           +h23[j]*et2r[j+ss-nf+2]+h24[j]*et2i[j+ss-nf+2];

                temp7=temp7+h31[j]*et1r[j+ss-nf+2]+h32[j]*et1i[j+ss-nf+2]
                           +h33[j]*et2r[j+ss-nf+2]+h34[j]*et2i[j+ss-nf+2];

                temp8=temp8+h41[j]*et1r[j+ss-nf+2]+h42[j]*et1i[j+ss-nf+2]
                           +h43[j]*et2r[j+ss-nf+2]+h44[j]*et2i[j+ss-nf+2];
            }
            et1r_o[ss] = temp1;
            et1i_o[ss] = temp2;
            et2r_o[ss] = temp3;
        ...
        for(int i=(s-2*cpewindow); i<(s+2*cpewindow); i=i+2){
            CorrWindowl[i-s+2*cpewindow] = cmp(et1r_o[i],et1i_o[i])*exp(-cmp(0,1)*cmp(phase1[s-2],0));
            CorrWindowr[i-s+2*cpewindow] = cmp(et2r_o[i],et2i_o[i])*exp(-cmp(0,1)*cmp(phase2[s-2],0));
        }
        ...
        templ = cmp(et1r_o[s],et1i_o[s])*exp(-cmp(0,1)*phase1[s]);
        tempr = cmp(et2r_o[s],et2i_o[s])*exp(-cmp(0,1)*phase2[s]);        
        
        /* decision and error on real&imag parts */
        error1r[s] = points[minLocFunc(templ.real(),points,points_length)]-templ.real();
        error1i[s] = points[minLocFunc(templ.imag(),points,points_length)]-templ.imag();
        error2r[s] = points[minLocFunc(tempr.real(),points,points_length)]-tempr.real();
        error2i[s] = points[minLocFunc(tempr.imag(),points,points_length)]-tempr.imag();
