...
P.Scope = 0;
P.CohRx = 0;
P.Symbols = 2^17-4;                           % Symbols to capture
...
P.RRCAst = 30;                              % Stop band attenuation for RRC Filter type 'Att'
...
%%% EQ options
P.RealEQ = 1;                               % RealEQ = 1 - will use 4x4EQ
P.SuperConvergence = 0;                     % run the equaliser many times
P.FilterLength = 7;                        % equaliser filter length
...
%% Normalise
CompensatedSig = Orthonormalise(temp, P);
temp = CompensatedSig;
...
for Channel = 1:length(Signal.Fchan)
    ...
    [CarrierRecovered, ~] = QAM_CPE_VV(temp, P);
    temp = CarrierRecovered;
    
    %% Plots
    figure(10);
    xy = 1.5;
    subplot(221); plot(Equalized.Et(1,2:2:end),'.','markersize',2); axis square; axis([-xy xy -xy xy]);
    subplot(222); plot(Equalized.Et(2,2:2:end),'.','markersize',2); axis square; axis([-xy xy -xy xy]);
