...
%
% Author: Sean Kilmurray, October 2012.
% Modified: Milen Paskov, December 2013
% Modified: Eric Sillekens, June 2019
...
switch P.RRCType
    ...
    case 'Taps'
        ...
        RRCSpec = fdesign.pulseshaping(SignalIn.Fs/SignalIn.Fb, 'Square Root Raised Cosine', 'N,Beta', P.RRCTaps, P.RollOff);
        hh = design(RRCSpec);                        % Design filter
        Offset = P.RRCOffset*pi/SignalIn.Fb;
        hh.Numerator = hh.Numerator.*exp(1j*Offset*(1:length(hh.Numerator)));
        
        SignalOut.Et(1,:) = cconv(hh.Numerator, SignalOut.Et(1,:),Nt);
        SignalOut.Et(2,:) = cconv(hh.Numerator, SignalOut.Et(2,:),Nt);
        
        SignalOut.Et = circshift(SignalOut.Et,[0,-P.RRCTaps/2]);
        
    case 'Att'
        if ~isfield(P, 'RRCAtt'),
            P.RRCAtt = 40;
        end
        RRCSpec = fdesign.pulseshaping(SignalIn.Fs/SignalIn.Fb, 'Square Root Raised Cosine', 'Ast,Beta', P.RRCAtt, P.RollOff);
        hh = design(RRCSpec);                        % Design filter
        Offset = P.RRCOffset*pi/SignalIn.Fb;
        hh.Numerator = hh.Numerator.*exp(1j*Offset*(1:length(hh.Numerator)));
...
%         SignalOut.Et(1,:) = cconv(hh.Numerator, SignalOut.Et(1,:),Nt);
%         SignalOut.Et(2,:) = cconv(hh.Numerator, SignalOut.Et(2,:),Nt);
        tempEt(1,:) = fftfilt(hh.Numerator, [SignalOut.Et(1,:),zeros(1,floor(length(hh.Numerator)/2))]);
        tempEt(2,:) = fftfilt(hh.Numerator, [SignalOut.Et(2,:),zeros(1,floor(length(hh.Numerator)/2))]);
        SignalOut.Et = tempEt(:,floor(length(hh.Numerator)/2)+1:end);
end
