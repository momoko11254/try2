...
% Modified to run on Nvidia GPU (CUDA) by Sean Kilmurray, May 2011
% Modified: Gabriele Liga, 1 December 2013
% Modified by Domanic Lavery for clarity, speed and input verification, 2 May 2014
%
% see also MANAKOV
...
if (~isfield(P,'SinglePrecision')||(P.SinglePrecision~=1))
    if (isfield(P,'verbose')&&(P.verbose>=1)), disp('Double precision NLSE (for accuracy)'), end
else
    if (isfield(P,'verbose')&&(P.verbose>=1)), disp('Single precision NLSE (for speed)'), end
    if isfield(P,'PMD')
        theta = single(theta);
        phi = single(phi);
    end
...
if isfield(P, 'PMD') && Np==2,
    if isfield(P, 'Gamma')
        if (isfield(P,'verbose')&&(P.verbose>=1)), disp('Using Two Axis Split Step NLSE with SPM, XPM and PMD'), end
        ...
        for n=1:Nz,
            ...
            aEt = abs(Et).^2;
            N=NLFactor*[aEt(1,:)+2/3*aEt(2,:); aEt(2,:)+2/3*aEt(1,:)];  % Nonlinear operator SPM & XPM
        ...
        if (isfield(P,'verbose')&&(P.verbose>=1)), disp('Using Two Axis Split Step NLSE with PMD'), end
        for n=1:Nz,
            Ef=rotation(theta(n),phi(n))*Ef;                            % Apply random polarisation rotation and phase shift
...
elseif isfield(P, 'Gamma') && Np==2,
    if (isfield(P,'verbose')&&(P.verbose>=1)), disp('Using Two Axis Split Step NLSE with SPM and XPM'), end
    NLFactor = dzEff*1i*P.Gamma;
    for n=1:Nz,
...
elseif isfield(P, 'Gamma') && Np==1,
    if (isfield(P,'verbose')&&(P.verbose>=1)), disp('Using Single Axis Split Step NLSE'), end
    NLFactor = dzEff*1i*P.Gamma;
    for n=1:Nz,
        Ef=Ef.*D;                                                       % Apply Dispersion operator for the first half step
        Et=fft(Ef,[],2);                                                % Fourier Transform to time domain
        N=NLFactor*abs(Et).^2;                                          % Nonlinear operator SPM & XPM
        Et=Et.*exp(N-dz*a/2);                                           % Apply Nonlinear and loss operators at center of step
        Ef=ifft(Et,[],2);                                               % Fourier Transform to frequency domain
        Ef=Ef.*D;                                                       % Apply Dispersion operator for the second half step
    end
...
function P=IOVerify(P)
    ...
    if (isfield(P,'GPU')&&P.GPU==1&&gpuDeviceCount<1)
        warning('This machine does not support a GPU; NLSE will be computed on the CPU')
        P.GPU=0;
        warningnum=warningnum+1;
    end

    if (warningnum&&isfield(P,'verbose')&&(P.verbose>=1)), fprintf(['<strong>NLSE warnings: ' num2str(warningnum) '\n</strong>']), end
end